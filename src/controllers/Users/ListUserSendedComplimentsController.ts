...
export class ListUserSendedComplimentsController {
    async handle(req: Request, res: Response): Promise<Response> {
        ...
        const listUserSendedComplimentsService = container.resolve(
            ListUserSendedComplimentsService
        );

        const compliments = await listUserSendedComplimentsService.execute(
            user_id
        );
