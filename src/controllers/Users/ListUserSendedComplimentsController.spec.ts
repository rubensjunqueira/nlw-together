...
describe('ListUserSendedCompliments', () => {
    beforeAll(async () => {
        await connect();
    });
    ...
    it('should not be able to list all user compliments if user is not authenticated with token', async () => {
        const response = await supertest(app).get('/users/compliments/sended');
    ...
    it('should be return status 200 if list contains itens', async () => {
        ...
        await supertest(app).post('/users/authenticate').send({
            email: user2.email,
            password: '123456',
        });
